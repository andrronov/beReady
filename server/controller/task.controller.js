...
class TaskController {
  async getUserTasks(req, res, next) {
    ...
    try {
      const tasks = await dbp.query("select * from tasks where user_id = $1", [
        id,
      ]);
      res.json(tasks);
    } catch (err) {
  ...
  async deleteTask(req, res, next) {
    const { id } = req.params;
    try {
      const deleted_task = await dbp.query("delete from tasks where id = $1", [
        id,
      ]);
